  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できない場合' do
      ...
      it '全角文字を含むパスワードでは登録できない' do
        @user.password = 'ｙopon2'
        @user.password_confirmation = 'ｙopon2'
        @user.valid?
        expect(@user.errors.full_messages).to include('パスワードは不正な値です')
      end
      ...
      it 'last_name_kanaが空では登録できない' do
        @user.last_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("カタカナの苗字を入力してください")
      end
      it 'first_name_kanaが空では登録できない' do
        @user.first_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("カタカナの名前を入力してください")
      end
      it 'last_name_kanaが半角では登録できない' do
        @user.last_name_kana = 'ﾏｴﾀﾞ'
        @user.valid?
        expect(@user.errors.full_messages).to include('カタカナの苗字は不正な値です')
      end
      it 'first_name_kanaが半角では登録できない' do
        @user.first_name_kana = 'ﾓﾓｺ'
        @user.valid?
        expect(@user.errors.full_messages).to include('カタカナの名前は不正な値です')
      end
      it 'last_name_kanaが漢字では登録できない' do
        @user.last_name_kana = '前田'
        @user.valid?
        expect(@user.errors.full_messages).to include('カタカナの苗字は不正な値です')
      end
      it 'first_name_kanaが漢字では登録できない' do
        @user.first_name_kana = '桃子'
        @user.valid?
        expect(@user.errors.full_messages).to include('カタカナの名前は不正な値です')
      end
